...
module Logan
  class Project
    ...
    # get topics from a project
    #
    # @return [Array<Logan::Topic>] array of topics (message / todo item)
    def topics
      completed_response = Logan::Client.get " /projects/#{@id}/topics.json"
      topic_array = completed_response.parsed_response.map do |h|
        Logan::Topic.new h.merge({ :project_id => @id })
      end
    end
  end
end